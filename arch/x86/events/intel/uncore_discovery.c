...
static int __parse_discovery_table(char *type,
				   struct uncore_discovery_domain *domain,
				 ...
				 u32 bar_offset, bool *parsed)
{
	...
	return __parse_discovery_table("PCI", domain, addr, die, parsed);
}
...
static bool uncore_discovery_msr(struct uncore_discovery_domain *domain)
{
	...
	for_each_online_cpu(cpu) {
		...
		__parse_discovery_table("MSR", domain, base, die, &parsed);
	}
